...
node scripts/verify/e2e.mjs              http://localhost:8787 all

# 3) 배포 후 프로덕션 최종 확인 (풀 E2E는 1회만 — 실제 과금)
node scripts/verify/verify-a11y.mjs      https://myofferagent.com
node scripts/verify/e2e.mjs              https://myofferagent.com all
```
...
| 스크립트 | 검증 항목 | 기준선 |
|---|---|---|
| `browser.mjs` | pdfjs 로컬 로딩(CDN 금지), CSP 위반, PDF 왕복(우리가 만든 PDF를 우리가 파싱), 패널 편집·영속화·내보내기 | 13건 |
